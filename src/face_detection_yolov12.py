    ...
    def draw_faces(self, image, detections, color=(0, 255, 0), thickness=2, show_confidence=True):
        ...
        for det in detections:
            ...
            if show_confidence:
                ...
                cv2.rectangle(result, (x1, y1 - text_size[0][1] - 10), (x1 + text_size[0][0] + 5, y1), color, -1)
                cv2.putText(result, conf_text, (x1 + 2, y1 - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
        return result
...
class WebcamFaceDetectionGUI:
    def __init__(self, model_path, conf_threshold=0.5, skip_frames=2, inference_width=640):
        ...
        self.status_label = Label(control_frame, text="Running YOLOv12", bg="lightgray", fg="green", font=("Arial", 10, "bold"))
    ...
    def update_frame(self):
        ...
        if should_detect:
            detections = self.detector.detect_faces_optimized(frame, self.conf_threshold, self.inference_width)
        ...
        for det in detections:
            ...
            cv2.putText(result, f"{conf:.1%}", (x1, y1 - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 1)
...
def detect_from_video(video_path, model_path, output_path=None, conf_threshold=0.5):
    ...
    writer = cv2.VideoWriter(str(output_path), cv2.VideoWriter_fourcc(*'mp4v'), fps, (width, height)) if output_path else None
...
if __name__ == "__main__":
    ...
    required_models = ["yolov12n-face.pt", "yolov12s-face.pt", "yolov12m-face.pt", "yolov12l-face.pt"]
